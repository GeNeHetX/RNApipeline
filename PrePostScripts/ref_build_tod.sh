...
Options:
  ...
  --force                  Replace an existing reference after preserving it
                           under /ref/.staging/.
  --keep-work              Keep the build directory on success.
  --ref-root PATH          Override /ref for testing or controlled staging.
  --work-root PATH         Override the build root.
  --skip-mount-check       Skip the CephFS mount check (tests only).
  --phase NAME             Internal Nextflow phase: prepare-inputs,
                           prepare-images, sequence-indexes, variant-index,
                           kallisto-index, gtf-artifacts, star-index, or
                           finalize.
...
case "$(uname -m)" in
    aarch64|arm64)
        KALLISTO_SIF="$KALLISTO_ARM_SIF_PATH"
        ;;
    x86_64)
        KALLISTO_SIF="$KALLISTO_AMD64_SIF_PATH"
        ;;
    *)
        KALLISTO_SIF=""
        ;;
...
cleanup() {
    ...
    if ((KEEP_WORK == 1 || rc != 0)) || [[ "$PHASE" != "all" ]]; then
        if [[ -n "${BUILD_DIR:-}" && -d "${BUILD_DIR:-}" ]]; then
            if [[ "$PHASE" == "all" ]]; then
                log "Keeping scratch build directory: ${BUILD_DIR}"
            else
                log "Keeping shared phase build directory: ${BUILD_DIR}"
            fi
        fi
...
pull_arch_image() {
    ...
    # This image may be foreign to the current worker (VEP is AMD64-only on
    # ARM workers). Defer execution validation to a matching architecture.
    if [[ -s "$target" ]]; then
...
validate_kallisto_sifs() {
    ...
    # Both files must be present for a portable reference bundle. Only the
    # selected image can be executed and validated on this worker.
...
write_kallisto_wrapper() {
    local wrapper="${KALLISTO_TOOL_ROOT}/bin/kallisto"
...
case "\$(uname -m)" in
    aarch64|arm64) IMAGE="\${ROOT_DIR}/arm64/kallisto-${KALLISTO_VERSION}.sif" ;;
    x86_64) IMAGE="\${ROOT_DIR}/amd64/kallisto-${KALLISTO_VERSION}.sif" ;;
...
prepare_reference_images() {
    ...
    # The upstream VEP image is AMD64-only. Keep it available for validation
    # on TOD even when this reference build runs on ARM64.
...
write_manifest() {
    ...
    export MANIFEST_KALLISTO_ARM64_SIF="$KALLISTO_ARM_SIF_PATH"
    export MANIFEST_KALLISTO_AMD64_SIF="$KALLISTO_AMD64_SIF_PATH"
...
containers["kallisto"].update({
    ...
    "arm64_build": "containers/kallisto/build-arm64-sif.sh",
    "architectures": {
        "arm64": {
            "path": os.path.join(os.environ["MANIFEST_KALLISTO_TOOL_ROOT"], "arm64", "kallisto-0.51.1.sif"),
            "sha256": sha256(Path(os.environ["MANIFEST_KALLISTO_ARM64_SIF"])),
        },
        "amd64": {
...
finalize_reference() {
    [[ -e "$FINAL_DIR" || -L "$FINAL_DIR" ]] && ((FORCE == 0)) \
    ...
    [[ -s "${FINAL_DIR}/reference_manifest.json" ]] \
        || die "published reference has no manifest"
...
main() {
    ...
    case "$PHASE" in
        all|prepare-inputs|prepare-images|sequence-indexes|variant-index|kallisto-index|gtf-artifacts|star-index|finalize)
            ;;
        *)
    ...
    check_ref_mount
    if [[ "$PHASE" == "all" || "$PHASE" == "prepare-inputs" ]]; then
        check_source_urls
    fi
    ...
    export APPTAINER_TMPDIR="${APPTAINER_TMPDIR:-${BUILD_DIR}/apptainer-tmp/${PHASE}-${CURRENT_JOB}}"
